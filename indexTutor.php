...
// Consulta principal: traer tutores con sus datos personales y alumnos asignados
$sql = "
SELECT 
    tutores.persona_idpersona,
    ...
    GROUP_CONCAT(CONCAT(alumnos_persona.apellido, ', ', alumnos_persona.nombre) SEPARATOR '<br>') AS alumnos_asignados
FROM tutores
JOIN persona ON tutores.persona_idpersona = persona.idpersona
LEFT JOIN alumnos_has_tutores ON tutores.idtutores = alumnos_has_tutores.tutores_idtutores AND tutores.persona_idpersona = alumnos_has_tutores.tutores_persona_idpersona
LEFT JOIN alumnos ON alumnos.idalumno = alumnos_has_tutores.alumnos_idalumno AND alumnos.persona_idpersona = alumnos_has_tutores.alumnos_persona_idpersona
LEFT JOIN persona alumnos_persona ON alumnos.persona_idpersona = alumnos_persona.idpersona
$whereClause
GROUP BY tutores.persona_idpersona
ORDER BY persona.apellido ASC, persona.nombre ASC
";
	...
	<div class="container">
		<div class="content">
			...
			if (isset($_GET['aksi']) && $_GET['aksi'] == 'delete') {
				...
				// Eliminar relaciones en alumnos_has_tutores
				mysqli_query($con, "DELETE FROM alumnos_has_tutores WHERE tutores_persona_idpersona='$nik'");

				$cek = mysqli_query($con, "SELECT * FROM tutores WHERE persona_idpersona='$nik'");
				if (mysqli_num_rows($cek) == 0) {
					...
					$delete = mysqli_query($con, "DELETE FROM tutores WHERE persona_idpersona='$nik'");
					if ($delete) {
				...
				<div class="form-group">
					<select name="dni" class="form-control" id="buscadorDni">
						...
						$dni_query = mysqli_query($con, "
                            ...
                            FROM persona 
                            INNER JOIN tutores ON persona.idpersona = tutores.persona_idpersona 
                            ORDER BY persona.dni ASC
                        ");
						while ($rowDni = mysqli_fetch_assoc($dni_query)) { ?>
							<option value="<?php echo htmlspecialchars($rowDni['dni']); ?>" <?php if (isset($dniBuscado) && $dniBuscado == $rowDni['dni']) echo 'selected'; ?>>
				...
				if ($query && mysqli_num_rows($query) > 0) {
					while ($row = mysqli_fetch_assoc($query)) { ?>
						<div class="card-personal">
							...
							<div class="info-row">
								<span class="info-label"><b>Alumnos asignados:</b></span><br />
								<span><?php echo $row['alumnos_asignados'] ? $row['alumnos_asignados'] : '<i>Sin alumnos asignados</i>'; ?></span>
							</div>
							...
							<div class="card-actions">
								...
								<a href="indexTutor.php?aksi=delete&nik=<?php echo $row['persona_idpersona']; ?>" title="Eliminar" onclick="return confirm('¿Está seguro de borrar los datos de <?php echo addslashes($row['personaApellido'] . ' ' . $row['personaNombre']); ?>?')" class="btn btn-danger btn-sm">
