...
if (isset($_POST['cargo']) && !empty($_POST['cargo']) && $_POST['cargo'] !== "Seleccione el Cargo") {
	$cargoBuscado = mysqli_real_escape_string($con, strip_tags($_POST['cargo'], ENT_QUOTES));
	$where[] = "cargos.cargo = '$cargoBuscado'";
}
...
$sql = "SELECT personal.*, 
               ...
               personal.persona_idpersona AS persona_idpersona, 
               GROUP_CONCAT(DISTINCT cargos.cargo SEPARATOR ', ') AS cargosOcupados,
               GROUP_CONCAT(DISTINCT titulos.titulo SEPARATOR ', ') AS titulosPersonal
        FROM personal 
        JOIN persona ON personal.persona_idpersona = persona.idpersona
        LEFT JOIN personal_has_cargos phc ON phc.personal_idpersonal = personal.idpersonal
        LEFT JOIN cargos ON phc.cargos_idcargos = cargos.idcargos
        LEFT JOIN titulos_personal tp ON tp.personal_idpersonal = personal.idpersonal AND tp.personal_persona_idpersona = personal.persona_idpersona
        LEFT JOIN titulos ON tp.titulos_idtitulos = titulos.idtitulos
        $whereClause
        GROUP BY personal.idpersonal
        ORDER BY personal.idpersonal ASC";
	...
	<div class="container">
		<div class="content">
			...
			if (isset($_GET['aksi']) && $_GET['aksi'] == 'delete') {
				...
				$cek = mysqli_query($con, "SELECT * FROM personal WHERE idpersonal='$nik'");
				if (mysqli_num_rows($cek) == 0) {
					...
					// Eliminar relaciones antes de borrar el personal
					mysqli_query($con, "DELETE FROM personal_has_cargos WHERE personal_idpersonal='$nik'");
					mysqli_query($con, "DELETE FROM titulos_personal WHERE personal_idpersonal='$nik'");
					mysqli_query($con, "DELETE FROM personal_has_cursos_armados WHERE personal_idpersonal='$nik'");
					$delete = mysqli_query($con, "DELETE FROM personal WHERE idpersonal='$nik'");
					if ($delete) {
						echo '<div class="alert alert-success alert-dismissable">
        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
        Datos eliminados correctamente.
        </div>';
						echo '<script>setTimeout(function(){ window.location.href = "indexPersonal.php"; }, 1200);</script>';
					} else {
        ...
        Error al eliminar los datos.
			...
			<form class="form-inline" method="post" action="">
				...
				<div class="form-group">
					<select name="dni" class="form-control" id="buscadorDni">
						...
						$dni_query = mysqli_query($con, "
                            ...
                            FROM persona 
                            INNER JOIN personal ON persona.idpersona = personal.persona_idpersona 
				...
				<!-- Buscador mediante Cargo -->
				<div class="form-group">
					<select name="cargo" class="form-control" id="buscadorCargo">
						<option value="Seleccione el Cargo">Seleccione el Cargo</option>
						<?php
						$cargos_query = mysqli_query($con, "SELECT DISTINCT cargo FROM cargos ORDER BY cargo ASC");
						while ($cargo = mysqli_fetch_assoc($cargos_query)) { ?>
							<option value="<?php echo htmlspecialchars($cargo['cargo']); ?>" <?php if (isset($cargoBuscado) && $cargoBuscado == $cargo['cargo']) echo 'selected'; ?>>
			...
			<div class="cards-container">
				...
				if ($query && mysqli_num_rows($query) > 0) {
					while ($row = mysqli_fetch_assoc($query)) { ?>
						<div class="card-personal">
							...
							<div class="info-row"><span class="info-label"><b>Titulo(s):</b></span> <span><?php echo htmlspecialchars($row['titulosPersonal'] ?: '-'); ?></span></div>
							<div class="info-row"><span class="info-label"><b>Cargo(s): </b></span> <span><?php echo htmlspecialchars($row['cargosOcupados'] ?: '-'); ?></span></div>
							<div class="info-row"><span class="info-label"><b>Inicio en el sistema:</b></span>
								<span>
									<?php
							...
							<div class="card-actions">
								...
								<a href="indexPersonal.php?aksi=delete&nik=<?php echo $row['idpersonal']; ?>" title="Eliminar" onclick="return confirm('¿Está seguro de borrar los datos de <?php echo addslashes($row['personaApellido'] . ' ' . $row['personaNombre']); ?>?')" class="btn btn-danger btn-sm">
				...
				<?php }
					...
					if (
						...
						(isset($_POST['cargo']) && $_POST['cargo'] !== "Seleccione el Cargo" && $_POST['cargo'] !== "")
					) {
